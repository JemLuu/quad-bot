...
- Use the recorded **Server name** (default `google-calendar`) and **Tool names** to build fully-qualified MCP tool calls, e.g. `google-calendar:create_event`. Don't hard-code tool names if `integrations.md` records different ones.
...
1. **Writes and deletes target ONLY the training calendar ID.** Never create, update, or delete events on the user's primary calendar or any other calendar.
2. **Reads may span calendars** — to find free time, you may read the user's other calendars' busy blocks (`list_events` / `suggest_time` / freebusy). Reading is fine; writing is not.
...
5. **Persist every event ID.** After `create_event`, write the returned event ID into `plan.md`'s Calendar sync state (Date | Session | Event ID). After `delete_event`, remove that row. This mapping is what makes updates idempotent and prevents duplicates.
...
4. For each approved session: `create_event` in the training calendar; record the event ID in `plan.md`.
5. To change a session later: `update_event` (or `delete_event` + `create_event`) using the stored ID; update the mapping.
